...
> 项目名：**privatedrop**。复用了 `/home/teemo/projects/thingspan` 的认证框架与前端 UI 组件库。
...
**v1（核心）**
- 单密码登录（复用 thingspan 认证），密码为环境配置，无注册/用户表
...
- **同源部署**：前端构建产物打进 FastAPI 静态目录（照搬 thingspan 的 Dockerfile 模式），一个应用容器对外服务，无 CORS 问题。
...
| 层 | 选型 | 来源 |
|----|------|------|
| 后端框架 | FastAPI + Pydantic v2 + SQLAlchemy 2 (async) | 与 thingspan 一致 |
| 认证 | JWT 双 token + bcrypt + refresh rotation + 内存吊销 | **直接复用 thingspan** `security.py` / `deps.py` / `auth.py` |
| 数据库 | PostgreSQL 16 + Alembic | 用户确认（原方案，替换 SQLite） |
| 对象存储 | MinIO（S3 兼容） | 预签名 URL、生命周期 |
| 实时推送 | WebSocket（FastAPI 进程内 ConnectionManager） | 单进程即可，无 Redis |
| 前端 | React 18 + Vite 5 + TS 5.5 + Tailwind 3 | 与 thingspan 完全同构 |
| 前端 UI | shadcn 风格自写组件（Radix + cva + tabler-icons） | **直接复用 thingspan** `src/components/ui/*` |
| 前端状态 | TanStack Query 5（服务端状态）+ 手写 useState | 与 thingspan 一致，不引 Zustand |
| 部署 | Docker Compose 单应用容器 + db + minio | 照 thingspan 单镜像合并模式 |
| 依赖管理 | uv（后端）+ pnpm/npm（前端） | 与 thingspan 一致 |

**从 thingspan 复用的清单（backend）**

| 文件 | 复用方式 |
|------|----------|
| `app/security.py` | 原样复制（bcrypt、JWT、吊销） |
| `app/api/deps.py` | 原样复制（`require_auth`） |
| `app/api/auth.py` | 复制后扩展：登录请求体加 `device_id`/`device_name`，登录时登记设备 |
| `app/schemas.py` 的认证 schema | 复制，`TokenResponse` 扩展 `device_id` |
| `app/config.py` 认证配置 | 复制 `app_password`/`jwt_secret`/token TTL，新增 minio/db 配置 |

**从 thingspan 复用的清单（frontend）**

| 文件 | 复用方式 |
|------|----------|
| `src/lib/utils.ts` (cn) | 原样复制 |
| `src/components/ui/*`（button/input/textarea/label/checkbox/select/dialog/card/badge/table） | 原样复制，零业务耦合 |
| `src/lib/api.ts` | 复制，改 localStorage key 前缀 `privatedrop_*` 与接口路径 |
| `src/lib/format.ts` | 原样复制（dayjs 工具） |
| `tailwind.config.js` + `src/index.css` | 原样复制（`:root` 主题变量） |
| `src/components/AppShell.tsx` | 改造：导航项/品牌名参数化，侧边栏 + 移动端 TabBar 布局沿用 |
| 缺失项（Toast/Confirm/Skeleton/Dropdown） | 本项目按需新增（沿用 Radix 模式）；删除确认暂用 `window.confirm` 与 things 一致 |

npm 依赖（things 同款）：`@radix-ui/react-{checkbox,dialog,label,select}`、`class-variance-authority`、`clsx`、`tailwind-merge`、`tailwindcss-animate`、`@tabler/icons-react`、`@tanstack/react-query`、`react-router-dom`、`dayjs`。

## 3. 认证与设备模型

沿用 thingspan 的**单密码单用户**设计：`APP_PASSWORD` 是唯一凭证（bcrypt 哈希存在进程内，启动时生成），无 User 表。

设备通过 JWT 载体识别（轻量扩展，不破坏 thingspan 的无状态设计）：
...
- 其余认证语义（access 15min、refresh 30 天、rotation、jti 吊销、`/api/auth/refresh` 需带有效 access）与 thingspan 完全一致
...
- 密码：`APP_PASSWORD` 环境配置，bcrypt 哈希；登录限流 5 次/分/IP（进程内 deque，同 thingspan）
- JWT：HS256 + `jwt_secret`；access 15min / refresh 30 天轮换；吊销列表在内存（重启即清，同 thingspan 取舍，单用户可接受）
...
- `.env` 不入库；`APP_PASSWORD`/`jwt_secret`/MinIO 凭证部署时必填并校验（照 thingspan 的 `validate_secrets`）
...
└── frontend/
    ...
    ├── tailwind.config.js     # ← 复制 thingspan
    ├── src/
    │   ├── index.css          # ← 复制（:root HSL 令牌）
    │   ├── main.tsx / App.tsx # RequireAuth + 路由
    │   ├── lib/
    │   │   ├── api.ts         # ← 复制改前缀；401 自动刷新单飞
    │   │   ├── utils.ts       # ← 复制
    │   │   ├── format.ts      # ← 复制
    │   │   └── types.ts       # Item / Device / WS 事件类型
    │   ├── components/
    │   │   ├── ui/            # ← 10 个组件整目录复制
    │   │   ├── AppShell.tsx   # ← 改造（导航/品牌参数化）
...
1. **单用户（复用 thingspan 认证）**：单密码无用户表，不引入账号体系；如需多用户，把 `security.py` 骨架 + 密码校验改为查 User 表即可，耦合度低。
2. **PostgreSQL 16**：用户确认。虽单用户 SQLite 够用，但 PG 为后续多用户/分享链接铺路；元数据量小，运维成本低。
3. **单镜像合并部署**：与 thingspan 一致，同源无 CORS，单容器即可上线；前后端仍保持独立代码目录与独立 dev 流程。
4. **无 Redis**：单进程下 WS/限流/吊销全部进程内（与 thingspan 的简洁一致）；`ws.py` 已抽象广播接口，将来拆 worker 时再补 Redis。
